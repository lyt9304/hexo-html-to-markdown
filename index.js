...
// PATH NEED TO BE HANDLE
var filePath = path.resolve('/Users/lyt9304/workspace/lyt9304.github.io/2018')
...
function writeFile(name, fileName) {
	if (path.extname(fileName) !== '.html') {
		// FIX: extname should be `.html`, not `html`
		return
	}
	...
	var docs =
		header +
		post
			.replace(/<td class="gutter">[\s\S]*?<\/td>/g, '')
			.replace(/(#* )\[\]\(#([\s\S]*?) "([\s\S]*?)"\)\3/g, '$1$2')
			.replace(
				/<figure[^>]*?>[\S\s]*?<pre>([\s\S]*?)<\/pre>[\s\S]*?<\/figure>/g,
				function(match, s1) {
					return (
						'```\n' +
	...
	console.log(path.join(__dirname, outUrl))
	// 写入内容
